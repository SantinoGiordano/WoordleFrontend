...
export default function Home() {
  ...
  const handleSubmit = () => {
    ...
    setGuesses((prev) =>
      prev.map((guess, index) => (currentValidation[index] ? guess : ""))
    );
    ...
    if (currentValidation.every((isCorrect) => isCorrect)) {
      ...
      const firstIncorrectIndex = currentValidation.findIndex((c) => !c);
    ...
    fetch(`${apiEndpoint}/api/words`)
      ...
      .then((animalNames) => {
        ...
        if (animalNames.length > 0) {
          const names = animalNames.map((a: Animal) => a.name.toLowerCase());
